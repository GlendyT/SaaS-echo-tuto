...
export const addFile = action({
  ...
  handler: async (ctx, args) => {
    ...
    const { entryId, created } = await rag.add(ctx, {
      ...
      contentHash: await contentHashFromArrayBuffer(bytes), // To avoid re-inserting if the file content has'nt changed
    });
...
async function convertEntryToPublicFile(
  ...
  if (storageId) {
    try {
      ...
      console.log("Failed to get storage matadata: ", error);
    }
  }
